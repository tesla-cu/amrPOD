import numpy as np
import matplotlib.pyplot as plt
from mpl_toolkits.axes_grid1 import AxesGrid

def comp_avg_rms(arr):

    avg = np.mean(arr)
    rms = np.std(arr - avg)
    return avg, rms

def Fig13_CPU(datadir, imgdir):

    print('making figure 13 ...')

    # Set up figure
    fig = plt.figure()

    # Set up data directories
    f1dir = datadir + 'AMR_gen/f1_3D/intel_timing/'
    f2dir = datadir + 'AMR_gen/f2_3D/intel_timing/'
    f3dir = datadir + 'AMR_gen/f3_3D/intel_timing/'

    # Data information
    ntmin = 10
    ntmax = 80
    ntinc = 10
    nt = np.arange(ntmin, ntmax+ntinc, ntinc)

    # Image set up
    grid = AxesGrid(fig, (0.08,0.075,0.91,0.89),
        nrows_ncols = (3, 3),
        axes_pad    = 0.2,
        aspect      = False,
        label_mode  = "L",
        share_all   = True,
        direction   = "column",
        cbar_mode   = "none")

    # Initialize data to store CPU time
    #  - index 1: nt
    #  - index 2: num of finest levels
    #  - index 3: first is avg, second is rms
    R_AMR    = np.zeros((len(nt), 3, 2))
    R_std    = np.zeros((len(nt), 3, 2))
    Phi_AMR1 = np.zeros((len(nt), 3, 2))
    Phi_AMR2 = np.zeros((len(nt), 3, 2))
    Phi_std  = np.zeros((len(nt), 3, 2))
    A_AMR    = np.zeros((len(nt), 3, 2))
    A_std    = np.zeros((len(nt), 3, 2))

    for i, n in enumerate(nt):
        for j in range(3):
            if j==0:
                fold = (f1dir + 'CPU_timing_nt%i/' % n)
            elif j==1:
                fold = (f2dir + 'CPU_timing_nt%i/' % n)
            elif j==2:
                fold = (f3dir + 'CPU_timing_nt%i/' % n)

            # CPU times for R
            CPUs = np.loadtxt(fold + 'R_CPU_AMR.txt')
            R_AMR[i,j,:] = comp_avg_rms(CPUs)
            CPUs = np.loadtxt(fold + 'R_CPU_standard.txt')
            R_std[i,j,:] = comp_avg_rms(CPUs)

            # CPU times for Phi
            CPUs = np.loadtxt(fold + 'Phi_CPU_AMR1.txt')
            Phi_AMR1[i,j,:] = comp_avg_rms(CPUs)
            CPUs = np.loadtxt(fold + 'Phi_CPU_AMR2.txt')
            Phi_AMR2[i,j,:] = comp_avg_rms(CPUs)
            CPUs = np.loadtxt(fold + 'Phi_CPU_standard.txt')
            Phi_std[i,j,:] = comp_avg_rms(CPUs)

            # CPU times for A
            CPUs = np.loadtxt(fold + 'A_CPU_AMR.txt')
            A_AMR[i,j,:] = comp_avg_rms(CPUs)
            CPUs = np.loadtxt(fold + 'A_CPU_standard.txt')
            A_std[i,j,:] = comp_avg_rms(CPUs)


    # Plot R CPU times for f1 -------------------------------------------------
    ax = grid[0].axes
    ax.plot(nt, R_std[:,0,0], linestyle='-', \
        color='k', label='$\overline{T}_s$')
    ax.plot(nt, R_AMR[:,0,0], linestyle='--', \
        color='b', label='$\overline{T}_a$')
    ax.set_title(r'(a) $\bf{R}$, $f=1$', loc='left')
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.grid('on', linestyle=':')
    
    # Label information
    ax.set_xticks(nt)
    ax.set_xticklabels(['10','20','','40','','60','','80'])
    ax.set_xlabel(r'$n_t$')
    ax.set_ylabel(r'$\overline{T}$ [CPU (s)]')
    ax.set_xlim(10,80)
    ax.legend(loc=2, frameon=True, facecolor='white', framealpha=1.0)

    # Plot R CPU times for f2 -------------------------------------------------
    ax = grid[1].axes
    ax.plot(nt, R_std[:,1,0], linestyle='-', \
        color='k', label='$\overline{T}_s$')
    ax.plot(nt, R_AMR[:,1,0], linestyle='--', \
        color='b', label='$\overline{T}_a$')
    ax.set_title(r'(d) $\bf{R}$, $f=2$', loc='left')
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.grid('on', linestyle=':')
    
    # Label information
    ax.set_xticks(nt)
    ax.set_xticklabels(['10','20','','40','','60','','80'])
    ax.set_xlabel(r'$n_t$')
    ax.set_ylabel(r'$\overline{T}$ [CPU (s)]')
    ax.set_xlim(10,80)

    # Plot R CPU times for f3 -------------------------------------------------
    ax = grid[2].axes
    ax.plot(nt, R_std[:,2,0], linestyle='-', \
        color='k', label='$\overline{T}_s$')
    ax.plot(nt, R_AMR[:,2,0], linestyle='--', \
        color='b', label='$\overline{T}_a$')
    ax.set_title(r'(g) $\bf{R}$, $f=3$', loc='left')
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.grid('on', linestyle=':')
    
    # Label information
    ax.set_xticks(nt)
    ax.set_xticklabels(['10','20','','40','','60','','80'])
    ax.set_xlabel(r'$n_t$')
    ax.set_ylabel(r'$\overline{T}$ [CPU (s)]')
    ax.set_xlim(10,80)

    # Plot Phi CPU times for f1 -----------------------------------------------
    ax = grid[3].axes
    ax.plot(nt, Phi_std[:,0,0], linestyle='-', \
        color='k', label='$\overline{T}_s$')
    ax.plot(nt, Phi_AMR1[:,0,0], linestyle='--', \
        color='b', label='$\overline{T}_a$ -- M1')
    ax.plot(nt, Phi_AMR2[:,0,0], linestyle=':', \
        color='orange', label='$\overline{T}_a$ -- M2')
    ax.set_title(r'(b) $\bf{\Phi}$, $f=1$', loc='left')
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.grid('on', linestyle=':')
    
    # Label information
    ax.set_xticks(nt)
    ax.set_xticklabels(['10','20','','40','','60','','80'])
    ax.set_xlabel(r'$n_t$')
    ax.set_ylabel(r'$\overline{T}$ [CPU (s)]')
    ax.set_xlim(10,80)
    ax.legend(loc=2, frameon=True, facecolor='white', framealpha=1.0)

    # Plot Phi CPU times for f2 -----------------------------------------------
    ax = grid[4].axes
    ax.plot(nt, Phi_std[:,1,0], linestyle='-', \
        color='k', label='$\overline{T}_s$')
    ax.plot(nt, Phi_AMR1[:,1,0], linestyle='--', \
        color='b', label='$\overline{T}_a$ -- M1')
    ax.plot(nt, Phi_AMR2[:,1,0], linestyle=':', \
        color='orange',label='$\overline{T}_a$ -- M2')
    ax.set_title(r'(e) $\bf{\Phi}$, $f=2$', loc='left')
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.grid('on', linestyle=':')
    
    # Label information
    ax.set_xticks(nt)
    ax.set_xticklabels(['10','20','','40','','60','','80'])
    ax.set_xlabel(r'$n_t$')
    ax.set_ylabel(r'$\overline{T}$ [CPU (s)]')
    ax.set_xlim(10,80)

    # Plot Phi CPU times for f3 -----------------------------------------------
    ax = grid[5].axes
    ax.plot(nt, Phi_std[:,2,0], linestyle='-', \
        color='k', label='$\overline{T}_s$')
    ax.plot(nt, Phi_AMR1[:,2,0], linestyle='--', \
        color='b', label='$\overline{T}_a$ -- M1')
    ax.plot(nt, Phi_AMR2[:,2,0], linestyle=':', \
        color='orange',label='$\overline{T}_a$ -- M2')
    ax.set_title(r'(h) $\bf{\Phi}$, $f=3$', loc='left')
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.grid('on', linestyle=':')
    
    # Label information
    ax.set_xticks(nt)
    ax.set_xticklabels(['10','20','','40','','60','','80'])
    ax.set_xlabel(r'$n_t$')
    ax.set_ylabel(r'$\overline{T}$ [CPU (s)]')
    ax.set_xlim(10,80)

    # Plot A CPU times for f1 -------------------------------------------------
    ax = grid[6].axes
    ax.plot(nt, A_std[:,0,0], linestyle='-', \
        color='k', label='$\overline{T}_s$')
    ax.plot(nt, A_AMR[:,0,0], linestyle='--', \
        color='b', label='$\overline{T}_a$')
    ax.set_title(r'(c) $\bf{A}$, $f=1$', loc='left')
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.grid('on', linestyle=':')
    
    # Label information
    ax.set_xticks(nt)
    ax.set_xticklabels(['10','20','','40','','60','','80'])
    ax.set_xlabel(r'$n_t$')
    ax.set_ylabel(r'$\overline{T}$ [CPU (s)]')
    ax.set_xlim(10,80)
    ax.legend(loc=2, frameon=True, facecolor='white', framealpha=1.0)

    # Plot A CPU times for f2 -------------------------------------------------
    ax = grid[7].axes
    ax.plot(nt, A_std[:,1,0], linestyle='-', \
        color='k', label='$\overline{T}_s$')
    ax.plot(nt, A_AMR[:,1,0], linestyle='--', \
        color='b', label='$\overline{T}_a$')
    ax.set_title(r'(f) $\bf{A}$, $f=2$', loc='left')
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.grid('on', linestyle=':')
    
    # Label information
    ax.set_xticks(nt)
    ax.set_xticklabels(['10','20','','40','','60','','80'])
    ax.set_xlabel(r'$n_t$')
    ax.set_ylabel(r'$\overline{T}$ [CPU (s)]')
    ax.set_xlim(10,80)

    # Plot A CPU times for f3 -------------------------------------------------
    ax = grid[8].axes
    ax.plot(nt, A_std[:,2,0], linestyle='-', \
        color='k', label='$\overline{T}_s$')
    ax.plot(nt, A_AMR[:,2,0], linestyle='--', \
        color='b', label='$\overline{T}_a$')
    ax.set_title(r'(i) $\bf{A}$, $f=3$', loc='left')
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.grid('on', linestyle=':')
    
    # Label information
    ax.set_xticks(nt)
    ax.set_xticklabels(['10','20','','40','','60','','80'])
    ax.set_xlabel(r'$n_t$')
    ax.set_ylabel(r'$\overline{T}$ [CPU (s)]')
    ax.set_xlim(10,80)

    # Save figure -------------------------------------------------------------
    fig.set_size_inches(6.5,5.5,forward=True)
    plt.savefig(imgdir + 'Fig13_CPU.png', dpi=600)    

    print('\tdone with figure 13')